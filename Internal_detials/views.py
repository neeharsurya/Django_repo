...
def Main_Home_page(request):
    ...
    Detail_3.img='Individual_comp.jpg'
...
def MCB_pdf_view(request):
    pdf_url='static/Internal_detials/pdf/Python Developer Master Catalog.pdf'
    with open(pdf_url,'rb') as pdf:
        response=HttpResponse(pdf.read(),content_type='application/pdf')
        response['Content-Disposition']='inline;filename=Python Developer Master Catalog.pdf'
        return response
